...
| `getResourceCitation` / `putResourceCitation` / `deleteResourceCitation` | TASK-074：读取、整份写入或替换、删除资料的文献信息（作者/年份/刊物/卷期页/DOI/ISBN/摘要，见 4.16）。首次写入 201、替换 200 且必须携带 `expected_version`（缺失 428、不符 409）；**整份替换、不做字段级合并**，省略的字段被清空；删除用强版本头。只写 `resource_citations`，不改资料标题、来源、标签、心得、高亮、学习数据与原件；服务端不解析 DOI、不查外部数据库、不出网。界面与浏览器连接器另行安排 |
| `uploadSnapshotAsset` / `listSnapshotAssets` / `downloadSnapshotAsset` / `deleteSnapshotAsset` | TASK-039：冻结快照正文里的图片。一次传一张（≤10 MiB，PNG/JPEG/GIF/WebP，按字节判定，不收 SVG），写操作以**快照版本**为前置条件且不推进它；同一 `source_url` 幂等。正文一字不改写，映射按 `source_url` 旁路。字节存进既有受控文件目录，替换正文/删快照/删资料三条路径都会隔离字节。服务端不出网 |
...
| 415 | `ASSET_TYPE_UNSUPPORTED` | 字节不是 PNG/JPEG/GIF/WebP 之一（SVG 明确不冻结）。 |
...
| `resource_count` | int | R | ≥0，自动 | 否 | taxonomy（TASK-034）；`topic_id` 指向本主题的资料份数；只读、请求体不得携带；只出现在 `TopicUsage`（分类端点的 list/detail/create/update 响应），不出现在资料响应内嵌的 `Topic` 引用中 |

删除被引用 Topic 返回 409；客户端须先通过 resources 逐份重分配或清空 `topic_id`。`resource_count` 与该 409 的 `details.resource_count` 同义，只是不必等到删除失败才能看到。
...
| `resource_count` | int | R | ≥0，自动 | 否 | taxonomy（TASK-034）；`resource_tags` 中引用本标签的关联行数（一份资料带同一标签只计一次）；只读、请求体不得携带；只出现在 `TagUsage`（分类端点的 list/detail/create/update 响应），不出现在资料响应内嵌的 `Tag` 中 |

颜色不属于已确认需求，不进入当前对象。删除有关联的 Tag 返回 409，先逐份解除关联；`resource_count` 与该 409 的 `details.resource_count` 同义，只是不必等到删除失败才能看到。
...
| `content` | string / `这里记录个人理解。` | C/U | 去首尾后 1～2,000,000（TASK-063 起；此前 50,000）；不空 | 是 | notes；只存个人笔记，不混入 AI 内容；Markdown 文本，可含 `![替代文字](data:image/png\|jpeg\|gif\|webp;base64,…)` 内嵌图片——服务端不解析、不校验图片内容，只按字符数限制；图片字节不单独存储 |
| `version` | int | R | 默认 1 | 否 | notes |
| `created_at` / `updated_at` | instant | R | 自动 | 否 | notes |

第一阶段笔记不进入资料统一搜索；删除笔记是直接删除。TASK-070 起顶层独立心得列表可按**标题**搜索（`q`，标题＝正文首个非空行，见 2.3），这只是该集合内部的检索，笔记仍不进入资料统一搜索、资料下的心得列表也不接受 `q`。绑定资料(resource_id 非空)的笔记在资源整体删除时进入影响确认集合；独立心得(resource_id 为 null)不属于任何资源，不进入资源删除影响集合。独立心得经顶层 `/api/v1/notes` 新增/回看/编辑/删除，也可经 `attachNote` 后贴绑定到资料（此后在资料笔记集合中、纳入该资料删除影响集合）；已绑定心得经 `detachNote` 解除回独立（此后回到顶层集合、退出资源删除影响集合）。
...
| ContentSnapshot—SnapshotAsset | 快照 1 对图片 0..*；`(snapshot_id, source_url)` 复合唯一、存储键唯一 | 图片随快照级联删除，**且其字节一并进入隔离区** | 可按快照版本单独删除一张；正文被整份替换或删除时全部作废 |
| LearningResource—ResourceCitation | 资料 1 对文献信息 0..1（`UNIQUE(resource_id)`） | 文献信息随资料级联删除 | 单独删除文献信息不影响资料及其任何从属数据 |
...
**渲染语义（TASK-042 起）。** 详情页**渲染**这份 Markdown，不再只展示源码（源码视图保留为一个切换）。渲染以 **`html: false`** 运行：正文里的原始 HTML（`<script>`、`<iframe>`、`<img onerror>` 等）一律转义成字面文本，**不引入消毒器**。这与 **TASK-036 任务记录**里选择 Markdown 而非 HTML 的理由（「抓自开放网络的 HTML 属不可信输入，需永久维护消毒器；Markdown 让不可信内容退化为惰性文本」）**不完全相同**——那句话在该任务记录里，本节此前并未写过它：此前是「压根不进 DOM」的结构性保证，此后是「按配置转义」的配置性保证，依赖渲染器选项、链接协议校验与该依赖的版本三者同时成立，三者各有机器守卫。链接以 `target="_blank" rel="noreferrer noopener"` 打开；`javascript:`/`vbscript:`/非图片 `data:` 由渲染器的链接校验拒掉。

**图片的三条去向。** 正文里的图片若能按 `source_url` 匹配到 4.14 的已冻结资产，显示**本机那一份**（经取字节端点 fetch 后转 blob URL —— `<img src>` 打不到该端点，见 4.14）；匹配不到、且解析后是绝对 http(s) 地址的**按原址自动加载**（用户 2026-09-07 在知情下述代价后决定），只加 `referrerpolicy="no-referrer"` 作缓解，并带 `loading="lazy"`（因此「各发一次请求」在实际中可能被推迟、甚至因未滚动到而不发）。**匹配与加载都以解析后的绝对地址为准**：正文里的相对地址先按快照的 `captured_from_url` 解析（`new URL(src, base).href`）再查表——冻结表的键是采集时算好的绝对地址，不解析就既匹配不上也渲染不出。**第三条去向**：解析后仍不是绝对 http(s) 的地址（例如 `data:image/png`，以及快照没有 `captured_from_url` 时的相对地址）**不产生 `img` 元素**，只留可读的替代文本——这比本节其余规定更严，如实记于此。**代价如实记于此**：打开一份资料会向每个未冻结图片的图床各发一次请求，暴露阅读时间与 IP；`SameSite=None` 的 cookie 会随之带出；这在显示路径上撤销了采集侧 `credentials: 'omit'` 的决定。**正文一字不改写**——替换只发生在渲染管线内，原站地址作为溯源信息保留在快照里。
...
### 4.14 SnapshotAsset（resources 所有）

| API 名称 | 类型/示例 | C/U | 必填/可空/默认/限制 | 敏感 | 所有者与不变量 |
| --- | --- | --- | --- | --- | --- |
| `id` | uuid | R | 必有 | 否 | resources |
| `snapshot_id` | uuid | R | 必有 | 否 | resources；外键指向 `content_snapshots.id`，`ON DELETE CASCADE` |
| `source_url` | string | C | 必填，1～2048，必须是绝对 http(s) 地址 | 否 | resources；`(snapshot_id, source_url)` 复合唯一 |
| `media_type` | string | R | `image/png` / `image/jpeg` / `image/gif` / `image/webp` | 否 | resources；由字节判定，不采信调用方声明 |
| `size_bytes` | int | R | 1～10,485,760 | 否 | resources |
| `sha256` | string | R | 必有，64 字符 | 否 | resources；取字节时逐份复核 |
| `created_at` | instant | R | 写入时刻 | 否 | resources |

响应中**没有** `version`，也**没有**内部存储键：资产只有新增与删除两种写，没有就地修改，因此没有自己的版本；写操作的前置条件是**所属快照的版本**，那是调用方在界面上能看到的东西。上传成功**不推进**快照版本 —— 图片在正文旁边，不改变正文。

**为什么挂在快照上，而不是资料上。** 图片是**某一份正文**的组成部分。正文被整份替换后，旧图片与新正文不再对应，挂在快照上时「换正文 → 旧图作废」就是外键的自然语义；挂在资料上则需要另写一套一致性维护。既有两张表都装不下它：`content_snapshots.content` 是文本列，`original_files` 是 `UNIQUE(resource_id)` 且其 media-type 白名单里没有任何图片格式。

**正文永不改写；映射走旁路。** 冻结图片**不修改** `content_snapshots.content`，正文里的图片引用仍是原站地址。资产用 `source_url` 记下这张图在正文里的位置，渲染方按该地址把引用替换为本机资产。三条理由：① 改写正文意味着每传一张图就要整份 `PUT` 一次并携带版本前置条件，一篇 50 张图的文章会产生 50 次版本推进与随时可能失败的冲突链，中断即留下「正文改了一半」；② 原站地址是溯源信息，改写后再也无法回答这张图当初来自哪里；③ 旁路使降级天然存在 —— 没有资产时正文照常可读（图片指向原站，退化回冻结前的行为），有资产时渲染方替换。

**只收四种位图，明确不冻结 SVG。** 类型按**文件头字节**判定，不采信上传方声明的 `Content-Type`、地址后缀或原站响应头 —— 这三者都由不可信的外部页面决定。SVG 被排除是安全决定：它是可执行 XML，可携带脚本与外链，冻结一张 SVG 等于把外站脚本存进本机受控目录，而受控目录与本机 UI 同源。

**取字节的端点不能当作 `<img src>` 用。** 本机访问门禁（第 7 节）要求每个 `/api/v1/*` 请求带进程令牌且 `sec-fetch-dest: empty`；浏览器发出的图片请求 `sec-fetch-dest` 为 `image` 且不带任何自定义头，**必然被拒**。因此调用方须自行 `fetch()` 取回字节，再经 `URL.createObjectURL(blob)` 交给 `<img>`。**这是门禁按设计工作，不是缺陷。** 明确不采纳的两个替代方案：放宽门禁接受图片请求（削弱一道已加固的边界换渲染便利）、把令牌写进图片地址（令牌会进入浏览器历史、referrer 与前端日志）。

**三条销毁路径都必须带走字节。** 外键 `CASCADE` 删的是数据库行，**删不掉磁盘上的字节**。因此：整份替换正文、删除快照、删除资料这三条路径，都在同一事务内删除资产行，并在提交后把字节移入隔离区（`trash/`）；删除资料的影响清单为此新增只读计数 `snapshot_asset_count`，资产数量变化会改变 `impact_revision`，从而使预览时取得的确认令牌失效。资产字节与原件共用同一套受控目录与同一套 24 小时孤儿回收：行还在字节就受保护，行没了字节才可回收。

**上限与去重。** 单图 10 MiB，超限在流式读取中途即中断（`413 ASSET_TOO_LARGE`），不把超限内容读进内存或落到磁盘。**每份快照不设张数上限**（用户 2026-09-06 明确决定「尽量全冻结」，并接受单份资料可能占用数百 MB 本机空间）。同一快照内 `source_url` 重复的上传是**幂等**的：返回既有资产，不产生第二行、也不产生第二份字节。跨快照**不做**字节去重 —— 同一张图出现在两份资料里会存两份，理由是引用计数会让上述三条销毁路径都变成「减一判零再隔离」，复杂度与出错面显著上升。冻结的是**原始字节**，不做压缩、缩略图、格式转换或 EXIF 清洗。
...
| `style` | string / `mark` | C/U | 必填，默认 `mark`；枚举 `mark`（高亮，整段填色）/ `underline`（下划线） | 否 | highlights；标注的样式（TASK-093）；`PATCH` 可改，改样式是同一条高亮、不是新的一条 |
| `color` | string / `yellow` | C/U | 必填，默认 `yellow`；枚举 `yellow` / `green` / `blue` / `pink` | 否 | highlights；标注的颜色（TASK-093），高亮与下划线共用同一套四色；`PATCH` 可改；渲染的具体色值由阅读器定 |
| `note_id` | uuid / null | C/U | 创建时可空；`PATCH` 里可选：给 `null` 是解绑，**省略是不动**（TASK-093 起）；必须是**同一份资料**下的笔记；一条笔记最多配一条高亮 | 否 | highlights；外键指向 `notes.id`，`ON DELETE SET NULL` |
| `version` | int | R | 默认 1 | 否 | highlights；改绑/解绑与改样式/颜色会推进；与库里相同的值不算改动、不推进 |
...
**锚点不可变；能改的只有心得绑定与样子。** `PATCH` 只接受 `note_id`、`style`、`color` 三个字段（外加 `expected_version`），**至少给一个，省略的字段一律不动**；一个都不给是 `422 VALIDATION_ERROR`（没有要做的事）。`note_id` 给 `null` 是解绑；`style`/`color` 不可为 `null`（给 `null` 是 `422`，不是「恢复默认」）。TASK-071 时 `PATCH` 只有 `note_id` 一个字段，因此规定「省略即 422」，为的是漏字段的请求不该悄悄解开用户配好的心得；TASK-093 起有三个字段，「省略＝不动」才是守住同一条承诺的规则——漏掉任何字段都不会丢任何东西。让锚点可改会出现「同一条高亮指向完全不同的话却保留原有历史」；标另一段就是另一条高亮。换色、高亮⇄下划线是同一条高亮的版本化写，不是删一条建一条。
...
### 4.16 Citation（resources 所有）

| API 名称 | 类型/示例 | C/U | 必填/可空/默认/限制 | 敏感 | 所有者与不变量 |
| --- | --- | --- | --- | --- | --- |
| `id` | uuid | R | 必有 | 否 | resources |
| `resource_id` | uuid | R | 必有 | 否 | resources；`UNIQUE`，一份资料至多一条文献信息；外键 `ON DELETE CASCADE` |
| `item_type` | string / `JOURNAL_ARTICLE` | C/U | `JOURNAL_ARTICLE` / `PREPRINT` / `CONFERENCE_PAPER` / `BOOK` / `BOOK_CHAPTER` / `THESIS` / `REPORT` / `WEBPAGE` / `OTHER`，默认 `OTHER` | 否 | resources；取自 CSL/Zotero 的类型表，只保留个人文库真正会有的几种 |
| `authors` | string[] / `["LeCun, Y.","Bengio, Y."]` | C/U | 可空；至多 100 个，单个去首尾空白后 1～200 字符；**空数组视为未填，存 null** | 否 | resources；**顺序即作品印出的作者顺序**，不重排、不去重、不拆姓名 |
| `issued_year` | int / `2015` | C/U | 可空，1000～2200 | 否 | resources；越界的是笔误或解析事故 |
| `issued_date` | string / `2015-05-28` | C/U | 可空，1～32 | 否 | resources；页面上印的日期**原样保留、不解析** |
| `container_title` | string / `Nature` | C/U | 可空，1～500 | 否 | resources；刊物、会议论文集或书名，**不是**这份资料自己的标题 |
| `volume` / `issue` / `pages` | string | C/U | 各可空，1～50 | 否 | resources；`pages` 是自由文本而非范围（`12-30`、`e0123456`、`1, 4-9` 都合法） |
| `publisher` / `doi` | string | C/U | 各可空，1～200 | 否 | resources；`doi` 原样保存，服务端不解析、不请求、不校验存在性 |
| `isbn` | string | C/U | 可空，1～32 | 否 | resources；原样保存，不校验校验位 |
| `abstract` | string | C/U | 可空，1～20,000 | 是 | resources |
| `version` | int | R | 默认 1 | 否 | resources |
| `created_at` / `updated_at` | instant | R | 自动 | 否 | resources |

所有可写字符串字段的规则与 `LearningResource.title` 一致：去首尾空白，清空用显式 `null`，空串与纯空白 `422`——否则同一个「没填」会有两种写法，日后的搜索与导出都要各处理一遍。

**为什么另建一张表，而不给 `learning_resources` 加列。** 那张表回答的是「我怎么回到我存的东西」——标题、地址、来源名称、为什么留它——每份资料的每一列都有意义；文献信息回答的是另一个问题「这是什么作品、该怎么引用」，对大多数随手保存的网页**根本不存在**，而且是**整块到达**（由连接器一次抓下来）而不是逐字段由人填。把十来个可空列挂到核心表上，每一次资料列表查询都要为它们付行宽，且「整份替换」的写会和资料已有的字段级 `PATCH` 混在同一个入口里，语义分不清。分表还有一条现实理由：资料的来源互斥 CHECK（见 4.1）管的是它自己的列，文献信息放新表则该 CHECK **一字未改**。

**与 `title` / `source_url` 的分工。** 资料的 `title` 是用户在自己的库里认出它的名字（可能是「那篇讲注意力的论文」），`source_url` 是回到它的地址；`container_title` 是**它登在哪**，`doi`/`isbn` 是**这个作品的标识符**。三者不互相覆盖，也不互相校验：改了文献信息不会改资料标题，反之亦然。本阶段**不做**按作者/DOI 的搜索与排序，`authors` 是 JSON 数组、不建索引——存下来是为了以后能导出与检索，不是现在就提供检索。

**为什么 `authors` 是 JSON 数组而不是一张关联表。** 作者顺序是引用的一部分，关联表要额外的位置列与第二条写入路径才能保住顺序；而本阶段既不按作者搜索也不做作者实体（同名合并、ORCID 都属于别的任务），关联表买不到任何东西。代价如实记于此：数据库层因此**没有**对元素个数与长度的 CHECK——用 SQL 数 JSON 数组元素要依赖方言函数，用字节长度近似则既会误伤合法姓名（转义会膨胀）又管不住个数。形状由 `modules/citations/contracts.py` 把守，它是唯一的写入者。

**写入语义。** `putResourceCitation` 是**整份替换**，不是字段级合并：首次写入省略 `expected_version` 并返回 201；已有文献信息时必须携带，缺失 `428`、不匹配 `409 VERSION_CONFLICT`；对尚不存在的文献信息携带 `expected_version` 返回 `404 CITATION_NOT_FOUND`（调用方的认知已过期）。**省略的字段表示「这份作品没有该项」并清空原值**——若改成字段级合并，删掉一个写错的作者或一个手滑的 DOI 就无法表达，只能再发明一套「null 表示删除」的约定；而调用方手上本来就有它刚读到的完整记录。请求体为 `{}` 是合法的（一条什么都没记的文献信息），它仍需版本前置条件，因此不会是误触。内容完全相同的重写不推进 `version`（2.4 节的既有规则）。删除只移除文献信息，资料及其标签、心得、高亮、学习数据与原件一律不变；删除资料则按第 9 节连同文献信息一并删除，影响计数为此新增 `citation_count`（0 或 1）。

**本阶段的边界（TASK-074）。** 服务端**不发起任何对外网络请求**：不解析 DOI、不查 Crossref/DataCite、不校验 DOI 或 ISBN 是否真实存在，元数据全部由调用方提供——这与 4.13/4.14 的快照与图片是同一条边界。也**不做**引文格式导出（BibTeX/CSL），那需要先定字段到 CSL 变量的映射与样式，属另一个任务。本任务只交付后端存储与三个接口，因此交付时界面上看不到变化。
...
2. 服务端保存令牌摘要，并把令牌绑定到资料 ID/版本，以及按类型和 ID 排序的 OriginalFile、SnapshotAsset、Citation、Note、Highlight、StudyRecord、ActiveReviewPlan、ReviewRecord、ResourceTag 的 ID+版本（不可变历史用创建序号；SnapshotAsset 无自身版本，用 ID + 存储键）全集。只比较数量不够。
...
主题/标签有引用时默认 `409 TAXONOMY_IN_USE` 并只返回安全计数；TASK-034 起同一份计数也在分类端点的 `resource_count` 里常态可见，客户端不必靠试删来发现分类是否在用。主题重分配/清空只能逐份调用 resources 修改资料；taxonomy 不直接写 `topic_id`，不会静默级联删除资料。
...
| DELETE `/resources/{resource_id}/snapshot/assets/{asset_id}` | 删除一张图片 / resources | 无体，强版本头；204 | 204/403/404/409/428/500/503；行删除、字节进隔离区 |
| GET `/resources/{resource_id}/citation` | 读取文献信息 / resources | 无体；200 `Citation` | 200/403/404/500；只读 |
| PUT `/resources/{resource_id}/citation` | 写入或整份替换文献信息 / resources | `CitationWrite`；201 首次 / 200 替换 | 200/201/400/403/404/409/415/422/428/500；只写 `resource_citations`，省略字段被清空，不改资料与其他从属数据；服务端不出网 |
| DELETE `/resources/{resource_id}/citation` | 删除文献信息 / resources | 无体，强版本头；204 | 204/403/404/409/428/500；资料保留 |
...
| GET `/api/v1/notes` | 独立心得列表 / notes | 分页 + 可选 `q`（按标题搜索，见 2.3）；200 `NotePage` | 200/403/422/500；只返回 `resource_id` 为 null 的独立心得；分页计数按过滤后的结果集算 |
...
| PATCH `/resources/{resource_id}/highlights/{highlight_id}` | 改心得绑定、样式或颜色 / highlights | `HighlightPatch`（`note_id`/`style`/`color` 至少给一个 + `expected_version`，省略的不动）；200 `Highlight` | 200/400/403/404/409/415/422/428/500；锚点不可改 |
...
| `getResourceCitation` | `HOST_FORBIDDEN`, `LOCAL_TOKEN_REQUIRED`, `LOCAL_TOKEN_INVALID`, `RESOURCE_NOT_FOUND`, `CITATION_NOT_FOUND`, `UNKNOWN_ERROR` |
| `putResourceCitation` | `HOST_FORBIDDEN`, `LOCAL_TOKEN_REQUIRED`, `LOCAL_TOKEN_INVALID`, `REQUEST_ORIGIN_FORBIDDEN`, `MALFORMED_REQUEST`, `CONTENT_TYPE_UNSUPPORTED`, `VALIDATION_ERROR`, `RESOURCE_NOT_FOUND`, `CITATION_NOT_FOUND`, `VERSION_CONFLICT`, `VERSION_REQUIRED`, `UNKNOWN_ERROR` |
| `deleteResourceCitation` | `HOST_FORBIDDEN`, `LOCAL_TOKEN_REQUIRED`, `LOCAL_TOKEN_INVALID`, `REQUEST_ORIGIN_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `CITATION_NOT_FOUND`, `VERSION_CONFLICT`, `VERSION_REQUIRED`, `UNKNOWN_ERROR` |
...
| `deleteSnapshotAsset` | `HOST_FORBIDDEN`, `LOCAL_TOKEN_REQUIRED`, `LOCAL_TOKEN_INVALID`, `REQUEST_ORIGIN_FORBIDDEN`, `RESOURCE_NOT_FOUND`, `SNAPSHOT_NOT_FOUND`, `SNAPSHOT_ASSET_NOT_FOUND`, `VERSION_CONFLICT`, `VERSION_REQUIRED`, `STORAGE_PATH_UNAVAILABLE`, `UNKNOWN_ERROR` |
...
- `extension/src/shared/protocol.test.ts` 在测试期只读前端那份源码，**逐字比对**两个校验函数的函数体（连注释一并比），并核对两份里所有共享上限的数值（TASK-075 起含八个文献上限） —— 保证「两份一模一样」；
- `frontend/src/features/capture/protocol.test.ts` **不跨目录读文件**，改为把同一套规则**逐条钉成行为断言**（每条拒收用例对应后端 `parsed_url` 里的一条规则）—— 保证「这一份符合规格」。

合起来两个方向都有机器守着，但**能力不同，须如实分清**：

- 改**扩展**那一份：逐字比对（含函数上方的 JSDoc）只要两份不同步就红，无论放宽还是收紧；
- 改**前端**那一份：**放宽**已被用例钉住的规则会红；**新增或收紧**规则不会红 —— 这个方向仍靠人守。

前者发现不了「两份一起错」，后者能（它把规则绑在后端 `parsed_url` 的真实约束上），两者互补。
...
| `studypilot-capture-image-request` | 页面 → 内容脚本 | `{ url }` | TASK-040。页面**逐张**索取图片字节。内容脚本只服务本次载荷 `images` 里带来的地址，其余一律答 `not-offered`（见 14.6）。 |
| `studypilot-capture-image-result` | 内容脚本 → 页面 | `{ url, result }` | TASK-040。`result` 为 `{ ok: true, base64, mediaType }` 或 `{ ok: false, reason }`。字节以 base64 传递：消息通道会 JSON 序列化，`ArrayBuffer` 过不去。 |
| `studypilot-fetch-image` | 内容脚本 → service worker | `{ url }` | TASK-040。走 `chrome.runtime`，不经页面。service worker 是扩展**两处**出网点之一（另一处见 14.7）。 |
...
| `images` | string[] | TASK-040。正文里引用的图片地址，**绝对 http(s)**、已去重、一次采集最多 60 条，每条 ≤ 2048 且不含空白/控制字符/凭据。**允许 `#`**（与 `url` 不同：资产的 `source_url` 只是匹配键，后端不禁片段标识符）。空数组是正常状态，**不表示**用户拒绝了权限 —— 那件事发生在此之后，由页面按取回结果如实告知 |
| `pdf` | `CapturedPdf \| null` | TASK-080。抓到的文献 PDF 原件（见 14.7）。没抓到、或这一页不是文献时为 `null`。兼容旧暂存：缺这个字段等同 `null`。 |
| `pdf_problem` | string \| null | TASK-080。`pdf` 为 `null` **且确实试过**时说明原因，取值 `cross-origin` / `too-large` / `not-pdf` / `slow` / `failed` 五者之一；这一页本来就没声明 PDF（或不是文献）时为 `null`——那不是问题，是常态。**先由 popup 据此停下来问一句**（TASK-085，见 14.4），用户选择改存正文后，确认页再据它说明一次这次为什么存的是正文。 |
| `citation` | `CapturedCitation \| null` | TASK-075。采集时从页面**自己声明的元数据**里认出的文献信息；认不出就是 `null`（多数网页不是文献，这是常态而非失败）。兼容旧暂存：缺这个字段等同 `null`。字段与 4.16 的文献接口同名同义，页面确认后原样交给 `PUT /resources/{id}/citation`。**不含 `abstract`**：摘要动辄上万字，为它撑大消息载荷不值得，也不是用户在确认页会看的东西。 |

#### `CapturedPdf`（TASK-080）

| 字段 | 类型 | 约束 |
| --- | --- | --- |
| `name` | string | 去空白后非空，≤ 200，必以 `.pdf` 结尾。优先用 PDF 地址的末段（arXiv 的 `/pdf/1706.03762` → `1706.03762.pdf`）；末段是 `file`/`download`/`printable` 一类通用词时改用这一页的标题（PLOS 的地址是 `/article/file?id=…&type=printable`，照抄会让每篇都叫 `file.pdf`——真实站点实测发现）。只剔除控制字符与路径分隔字符，中文与全角标点照留，与 8.1 的后端口径一致 |
| `bytes` | number | 安全整数，1 ≤ bytes ≤ 26,214,400（25 MiB，与 8.1 的单文件上限同一个数） |
| `base64` | string | 仅 `A–Za–z0–9+/` 与至多两个结尾 `=`；**长度与补齐位数须与 `bytes` 严格自洽**（`ceil(bytes/3)*4` 且补齐位数 = `(3 - bytes % 3) % 3`）。两项合起来才能唯一确定原始字节数——只比长度分辨不出同一组里相差一两个字节的情形 |

字节以 base64 传递的理由与图片相同：消息通道会 JSON 序列化，`ArrayBuffer` 过不去。

#### `CapturedCitation`（TASK-075）

| 字段 | 类型 | 约束 |
| --- | --- | --- |
| `item_type` | string | 4.16 的九种之一；认不准一律 `OTHER`，不猜 |
| `authors` | string[] | 至多 100 位，每位去空白后非空且 ≤ 200 字；认不出作者时是**空数组**（不是 null） |
| `issued_year` | number \| null | 整数 1000–2200；越界一律 `null`，不送一个必被 422 的值过去 |
| `issued_date` | string \| null | ≤ 32，**原样保留**不解析（`2024-03`、`Spring 2024` 都有人这么写） |
| `container_title` | string \| null | ≤ 500（期刊名、会议名、书名） |
| `volume` / `issue` / `pages` | string \| null | 各 ≤ 50 |
| `publisher` / `doi` | string \| null | 各 ≤ 200 |
| `isbn` | string \| null | ≤ 32 |
...
扩展**安装时**只申请 `activeTab`、`scripting`、`storage`、`unlimitedStorage` 四项权限与**一条只匹配本机 UI 源**的内容脚本；**不申请永久的 `host_permissions`**。这四项**没有一项给出站点访问权**——`manifest.test.ts` 另有断言钉死 `permissions` 里不得出现 `://` 或 `<all_urls>`。`unlimitedStorage`（TASK-080）只解除扩展自己那块本地存储的默认约 10 MB 配额：抓下来的 PDF 要先落一次盘才能转交确认页，一篇论文常有几 MB，base64 后再涨三分之一（实测 6.45 MB 的 PDF 编成 8.61 MB），旧配额装不下。它不扩大任何网络可达范围，也不给任何站点的读写权。`activeTab` 只在用户点击扩展图标后授予当前那一个标签页，用完即失效——扩展因此只能读到用户主动指定的那一页，且只能读到该页**已经渲染出来的内容**。它不读 cookie、不接触任何第三方站点的登录态。

**TASK-040 起多一项可选权限**：`optional_host_permissions: ["<all_urls>"]`，用于取回正文里图片的字节。它**安装时不授予**，只有当这一页的正文里有图片、且用户在 popup 上点「连图片一并保存」时，才经 `chrome.permissions.request` 请求；可在扩展设置页随时撤销。拒绝或撤销后采集照常完成，图片保留原站地址。选可选权限而非永久权限的理由：两者**覆盖面完全相同**，可选权限只多出「安装时不要、可撤销」两项，因此这次没有以覆盖面换安全性。为何必须是 `<all_urls>` 而非具体域名：图片挂在哪个图床由页面决定，采集前无法预知，按站点逐个请求会在一次采集里弹出多个授权框。

**同时新增 background service worker**。它在 TASK-040 时是扩展唯一会主动发出网络请求的地方；TASK-080 起出网点变为**两处**（另一处是抓同域 PDF 的注入脚本，见 14.7），两处之外一律不许，由 `extension/src/boundaries.test.ts` 扫源码守着。理由是 MV3 下内容脚本的跨源请求受 CORS 管，而图床普遍不发 CORS 头，只有持 host 权限的 service worker 取得到（这正是 4.13 所说「只有扩展能可靠取回图片字节」的具体含义）。它的职责被限死为一件事：收到取图请求 → 取字节 → 回传；取图时 `credentials: 'omit'`，**不带任何站点凭证**，因此需要登录才能看到的图片取不到——这是「凭证始终由浏览器持有」的直接后果，不是缺陷。

**TASK-080 起，抓到 PDF 的那一次不再请求图片权限**：确认页在 PDF 分支里根本不碰图片
（存的是原件，不是网页正文），照旧询问只会让用户为一件不会发生的事授出 `<all_urls>`——
实测 PLOS ONE 那一页正是「正文 5 张图 + PDF 抓取成功」这一类。判断是
`payload.images.length > 0 && !payload.pdf`（`extension/src/popup/popup.ts` 的
`shouldAskAboutImages`，有用例钉住）。这条只**减少**请求的权限，不新增任何授权面。

**确认页上没有「改存网页正文」的入口**（TASK-084 起）：抓到 PDF 就存 PDF，图片权限也因此
不会在这条路上被请求。此前本节写过「用户若在确认页取消勾选、改存网页正文……」，那个勾选框
已随 TASK-084 删除（用户 2026-09-21：「如果是文献的话默认保存 pdf 原文就可以了，不要让用户
做太多选择」）；TASK-085 订正了该陈述，TASK-086 在 TASK-084 合并后清掉了当时的过渡措辞。

**TASK-085 起，PDF 没抓到时 popup 停下来问一句**：判据是「已认出文献」且 `pdf_problem` 非空
（`extension/src/popup/popup.ts` 的 `needsPdfDecision`），popup 显示失败原因并给两个动作——
「改存这一页的正文」（继续既有的正文路径，有图时仍走既有的图片询问）与「算了」（不交付、
不开确认页）。**为什么必须停在 popup 里而不是在确认页上说**：`deliverCapture` 一旦
`openConfirmPage()`，popup 就被浏览器关闭，之后往它上面写什么都看不见——TASK-080 曾给 popup
加过 PDF 回执，正是因此从未显示出来过。用户 2026-09-21 在两种做法（停下来等一下 / 自动退回）
的草图之间选定了前者。
...
~~扩展只采集**正文**，正文中的图片引用仍指向原站，**本阶段不冻结图片**~~——**该边界已由 TASK-039（后端资产存储）与 TASK-040（采集侧取回）关闭，见 4.14 与 14.6。** 保留原文以记录 TASK-038 交付时的实际状态。扩展只支持 **Chrome 与 Edge**（Chromium），不支持 Firefox 与 Safari。另有一处已知残留：用户点了采集却始终不打开确认页时，待交付的正文会留在扩展本地存储中，直到下次采集覆盖或下次交付时清除；两处面向用户的文档均已如实告知。
### 14.6 图片取回的消息流与信任边界（TASK-040）

扩展**不能**直接把图片字节交给后端（门禁只信任 UI 源），也不能把它们塞进 `chrome.storage`
（10 MB 配额，几张图就撑爆，且会把第三方站点的字节留在扩展存储里）。因此走**逐张按需**的消息流：

1. popup 提取正文时一并认出图片地址（只认行内式 `![](url)`、尖括号式与残留 `<img src>`；
   **不认引用式**，那类页面的图片保留原站地址），去重并截断到一次采集 60 张；
2. 用户在 popup 点「连图片一并保存」→ 请求权限 → 地址清单随载荷暂存、打开确认页；
3. `/capture` 页面先建资料、再写正文，**拿到快照版本之后**才开始要图片：每次向中转脚本发一条
   取图请求，中转脚本转给 service worker，字节以 **base64** 回传（消息通道会 JSON 序列化，
   `ArrayBuffer` 过不去），页面转成 Blob 后经 `uploadSnapshotAsset` 上传，带该快照版本作前置条件；
4. 处理完第 i 张才要第 i+1 张：峰值内存只与单张图相关，失败一张不影响其余。

**信任边界在中转脚本**：它跑在本机 UI 源上，而该源上的任何脚本都能向本窗口 postMessage。
因此它**只服务本次采集载荷里带来的那些地址**，别的一律拒绝（`not-offered`）——否则扩展就成了
一个绕过 CORS 的通用代理。页面侧同样对回传的字节做形状校验；真正的类型裁决在后端，
它按字节魔数判定，不采信任何一侧的声明（见 4.14）。

**降级是设计的一部分，不是异常**：拒绝授权、图片需要登录、超过 10 MiB、不是四种位图之一、
service worker 唤不醒、消息过大——一律记为「这张没冻上」，正文与其余图片照常保存，
页面如实显示冻结了几张、几张没冻上，并说明那几张仍指向原站。**不自动重试。**

### 14.7 文献 PDF 的抓取规则（TASK-080）

**产物形态**：认出这一页是文献、且抓到了 PDF 时，采集产物是 **FILE 资料**——原件就是那份 PDF，
在站内 PDF 阅读器里读；**不存网页正文快照、不存摘要、不留原页地址**。第 5 节禁止 FILE 资料带
`source_url`，用户 2026-09-21 明确接受这一后果（「有了 pdf 就不用再要拔下来 pdf 那个网页了」）。
文献信息照常经 `PUT /resources/{id}/citation` 存下——它对 FILE 资料可用。

**只在同域直取，跨域一概不取**：PDF 地址取自这一页**自己声明**的 `citation_pdf_url`，
由**注入脚本**在页面上下文里 `fetch`。同域请求不需要任何 host 权限，这是「点一下就好、零额外授权」的落点；
`target.origin !== location.origin` 时**在发请求之前**就返回 `cross-origin`，绝不替用户向第三方站点发出请求。
取字节时 `credentials: 'omit'`，与取图同源同理：不带任何站点凭证。
**不把它挪去 service worker**——那条路要够得着跨域就得申请更大的站点权力，用更高的授权代价换一个次要场景。

**抓不到时先停在 popup 上问一句**（TASK-085 起；在那之前是自动退回）：popup 显示下表里对应的
原因，用户点「改存这一页的正文」才走既有行为（WEB 资料 + 正文快照 + 图片冻结），点「算了」
则什么都不存。**确认页上仍会再说一次原因**——用户改存正文之后看到的那一页。五种原因：

| `pdf_problem` | 含义 |
| --- | --- |
| `cross-origin` | PDF 与文章页不同源（部分出版社把 PDF 放在 CDN）。**没有发出任何请求** |
| `too-large` | 超过 25 MiB，后端本就不收 |
| `not-pdf` | 取回的字节不以 `%PDF-` 开头。付费墙常回 200 + 一页 HTML，因此**不信服务器的 `Content-Type`**，只认字节本身 |
| `slow` | 下载超过 **20 秒**（`extract.ts` 的 `PDF_TIMEOUT_MS`；实测五篇论文下载 0.8–9.2 秒，定在约一倍余量上）。整次采集的预算同步从 15 秒提到 **30 秒**（`popup/capture.ts` 的 `runCapture`），且必须比下载时限宽——否则先超时的是外层，下载不自带时限的话，一份慢 PDF 会把整次采集拖超时——用户连网页正文都存不下，恰好违背本节「拿不到就退回存正文」这条设计。超时单列而不并进 `failed`：前者「再试一次也许就成」，后者多半是付费墙，该怎么办完全不同 |
| `failed` | 非 2xx、网络失败、被拦截器掐断。**不猜原因**——分不清就说分不清 |

**只在已认出文献时才抓**：普通网页不该因为页面上有个 PDF 链接就被存成 PDF 资料。未认出文献的页面
行为与 TASK-080 之前完全一致。

**抓到 PDF 的那一次不问图片权限**，理由与代价见 14.4。

**暂存的代价如实记在这里**：PDF 的 base64 会随载荷在 `chrome.storage.local` 里停留到确认页取走为止
（这正是需要 `unlimitedStorage` 的原因）。用户点了采集却始终不打开确认页时，14.5 说的那份残留
从「一篇正文」变成「一篇正文 + 一份几 MB 的 PDF」，直到下次采集覆盖或下次交付时清除。
与 14.6 不取图入存储的理由不冲突：那里存的是**第三方图床**的字节且张数不定，这里存的是
**用户明确要保存的那一份同域原件**，恰好一份。
