...
TASK-011/012 已完成分类后端与页面，TASK-013 已合并 FILE 上传/保存/恢复/下载后端。TASK-014 接入文件上传/下载页面，随该任务测试、独立审查、验收通过并由用户合并后交付；本节只更新交付可用性，不改变标准契约或后端操作清单。
...
- TASK-013 的单任务证据记录文件实现、测试、独立 Review 和验收；上述交付元数据与代码一起审查，不能仅改清单就声称实现。资料修改/删除、学习等未列能力仍未开放；最终 MVP 验收仍要求完整目标通过。本节不授权回退已交付能力或改变 schema。
