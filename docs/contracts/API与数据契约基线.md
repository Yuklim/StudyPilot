...
| `studypilot-fetch-image` | 内容脚本 → service worker | `{ url }` | TASK-040。走 `chrome.runtime`，不经页面。service worker 是扩展唯一发出网络请求的地方。 |
...
扩展**安装时**只申请 `activeTab`、`scripting`、`storage` 三项权限与**一条只匹配本机 UI 源**的内容脚本；**不申请永久的 `host_permissions`**。`activeTab` 只在用户点击扩展图标后授予当前那一个标签页，用完即失效——扩展因此只能读到用户主动指定的那一页，且只能读到该页**已经渲染出来的内容**。它不读 cookie、不接触任何第三方站点的登录态。
...
**TASK-079 新增第十个键 `version_name`**：人看的版本标识，构建时写入 `<version>+<commit 短 SHA>`（拿不到 git 时为 `+dev`）。它**不扩大任何授权面**，加它是因为 `version` 是手写常量、改多少次代码都不动，用户无法判断 Chrome 里装的是哪一版。Chrome 的 `version` 只接受数字点分格式，塞不进 SHA；`version_name` 没有该限制且在扩展详情页直接可见。
