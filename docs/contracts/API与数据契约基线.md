...
| `pdf_problem` | string \| null | TASK-080。`pdf` 为 `null` **且确实试过**时说明原因，取值 `cross-origin` / `too-large` / `not-pdf` / `failed` 四者之一；这一页本来就没声明 PDF（或不是文献）时为 `null`——那不是问题，是常态。页面据此告诉用户这次为什么退回存正文。 |
