...
| `pdf_problem` | string \| null | TASK-080。`pdf` 为 `null` **且确实试过**时说明原因，取值 `cross-origin` / `too-large` / `not-pdf` / `slow` / `failed` 五者之一；这一页本来就没声明 PDF（或不是文献）时为 `null`——那不是问题，是常态。页面据此告诉用户这次为什么退回存正文。 |
...
**TASK-085 订正**（**生效前提：TASK-084 先合并**，见下）：上一版这里写着「用户若在确认页
**取消勾选**、改存网页正文，那一次的图片保留原网站地址……确认页在取消勾选的那一刻如实说明」。
**那个勾选框由 TASK-084 删除**
（用户 2026-09-21：「如果是文献的话默认保存 pdf 原文就可以了，不要让用户做太多选择」），
该陈述随之失效。TASK-084 因契约命中 high_risk_paths、改它要升 L3 而未同步，当时如实登记为
「已知的、写明的不一致」；本节在此订正。**TASK-084 合并之后的事实是**：抓到 PDF 就存 PDF，
确认页上没有改存网页正文的入口；图片权限也因此不会在这条路上被请求。

> **合并顺序要求（独立 Review F2）**：本节这段订正描述的是 TASK-084 的行为，而 TASK-084 当时
> 尚未合并。**TASK-085 必须在 TASK-084 之后合并**，否则这段话在 main 上是假的
> （AGENTS.md §5：未合并的依赖不得被当作已批准）。
