...
**TASK-085 订正**：上一版这里写着「用户若在确认页**取消勾选**、改存网页正文，那一次的图片
保留原网站地址……确认页在取消勾选的那一刻如实说明」。**那个勾选框已被 TASK-084 删除**
（用户 2026-09-21：「如果是文献的话默认保存 pdf 原文就可以了，不要让用户做太多选择」），
该陈述随之失效。TASK-084 因契约命中 high_risk_paths、改它要升 L3 而未同步，当时如实登记为
「已知的、写明的不一致」；本节在此订正。**现在的事实是**：抓到 PDF 就存 PDF，确认页上没有
改存网页正文的入口；图片权限也因此不会在这条路上被请求。
...
**抓不到就退回既有行为**（WEB 资料 + 正文快照 + 图片冻结），并在确认页说清原因。五种原因：
