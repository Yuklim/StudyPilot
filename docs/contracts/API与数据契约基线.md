...
- 后续原始文件任务必须实现既定 FILE 上传/保存/校验/恢复要求，补足测试、独立 Review 和验收，再同步更新本节、OpenAPI 清单及 README，解除暂时限制。不能仅改清单就声称实现；最终 MVP 验收仍要求完整 FILE 契约通过。本节也不授权回退已经交付的能力、改变 schema 或长期免除最终功能。
