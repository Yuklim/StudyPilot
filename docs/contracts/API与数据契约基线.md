...
- TASK-013～017 及 TASK-019 的单任务证据记录相应实现、测试、独立 Review 和验收；上述交付元数据与代码一起审查，不能仅改清单就声称实现。资料修改/删除、复习、统计等未列能力仍未开放；交付完整接口全集时才验该完整目标，当前核心验收以 1.0 和产品需求为准。本节不授权回退已交付能力或改变 schema。
