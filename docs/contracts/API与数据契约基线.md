...
**渲染语义（TASK-042 起）。** 详情页**渲染**这份 Markdown，不再只展示源码（源码视图保留为一个切换）。渲染以 **`html: false`** 运行：正文里的原始 HTML（`<script>`、`<iframe>`、`<img onerror>` 等）一律转义成字面文本，**不引入消毒器**。这与 4.13 原先「Markdown 让不可信内容退化为惰性文本」的性质**不完全相同**：此前是「压根不进 DOM」的结构性保证，此后是「按配置转义」的配置性保证，依赖渲染器选项、链接协议校验与该依赖的版本三者同时成立，三者各有机器守卫。链接以 `target="_blank" rel="noreferrer noopener"` 打开；`javascript:`/`vbscript:`/非图片 `data:` 由渲染器的链接校验拒掉。

**图片的两条去向。** 正文里的图片若能按 `source_url` 匹配到 4.14 的已冻结资产，显示**本机那一份**（经取字节端点 fetch 后转 blob URL —— `<img src>` 打不到该端点，见 4.14）；匹配不到的**按原址自动加载**（用户 2026-09-07 在知情下述代价后决定），只加 `referrerpolicy="no-referrer"` 作缓解。**代价如实记于此**：打开一份资料会向每个未冻结图片的图床各发一次请求，暴露阅读时间与 IP；`SameSite=None` 的 cookie 会随之带出；这在显示路径上撤销了采集侧 `credentials: 'omit'` 的决定。**正文一字不改写**——替换只发生在渲染管线内，原站地址作为溯源信息保留在快照里。
