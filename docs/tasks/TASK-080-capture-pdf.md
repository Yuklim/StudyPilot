...
### 范围修订 3（2026-09-21，L3 独立验收之后）

验收指出：`capturePdf` 没有自己的超时，下载算在 `runCapture` 的 15 秒总预算里，慢 PDF 会让
**整次采集**超时（什么都不交付）——用户连网页正文都拿不到，恰好违背本任务「拿不到 PDF 就退回
存正文」这条核心承诺。验收建议记为已知限制；**主 Agent 判断应当修**，理由是本机走代理，
实测 arXiv `1706.03762`（2.11 MB）下载就要 9,243 ms，触发概率不低，且后果是数据完全没存下。

由此产生两处**超出原登记范围**的改动，明示登记：

1. **`pdf_problem` 多出第五种取值 `slow`**（原登记与契约只有四种）。不并进 `failed`：超时是
   「再试一次也许就成」，`failed` 多半是付费墙，该怎么办完全不同——`pdf_problem` 存在的意义
   就是不把已经知道的信息丢掉。完成条件里「四种情形」一句同步改为五种。
2. **`runCapture` 的整次预算 15 秒 → 30 秒**（`extension/src/popup/capture.ts`，在范围内，
   但它是**对所有采集生效**的行为变更，不只是 PDF 路径）。已请独立验收专门判断是否越权，
   结论是「不超范围、不必退回」：外层宽于内层是修复本任务自身引入风险的必要条件。
   代价如实写明：本就卡住的普通采集要多等 15 秒才被告知失败，等待期间 popup 有提示。

时限取值由实测定，**且首版被实测否掉**：先定 10 秒，量出最慢一篇 9,243 ms 后改为 20 秒
（约一倍余量），外层 30 秒。「外层必须比内层宽」原先只活在注释里，现由
`capture.test.ts` 的守卫钉住（变异验证：预算调回 15 秒 → 变红）。

## 完成条件

- arXiv 这类同域 PDF：**一次点击、无额外授权**，产物是 FILE 资料，原件是那份 PDF，打开即进站内阅读器；文献信息一并存下。有用例。
- 拿不到 PDF 的**五种**情形（跨域无授权 / 403 或付费墙 / 超 25 MiB / 下载到的不是 PDF / **下载超时**）各自退回快照，并在确认页说清原因。各有用例。（第五种见「范围修订 3」——登记时没想到下载本身会拖垮整次采集。）
