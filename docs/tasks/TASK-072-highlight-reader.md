...
- Acceptance：L2，N/A。
- 最终状态/风险/用户操作：**MERGED**——2026-09-20 用户已合并 PR #80，merge commit `92a4764`；用户真机看过后「现在的页面没什么大问题」，两条待定的界面取舍（窄屏是否展开右栏、入口文案）**按用户决定保持现状不改**。风险低：纯前端，后端与契约一字未动；既有阅读器行为（Esc、焦点、⌘J、角标、窄屏浮层）由既有用例守住且全绿。用户操作：合并 PR，然后**在真机上试一次**——下面两条界面取舍要你看过才好定。
- 非阻断遗留项：
