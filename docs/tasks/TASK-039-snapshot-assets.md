...
- **非阻断遗留项（按 `风险分级与检查规则.md:29` 的四要素登记；清单由独立 Acceptance 给出，主 Agent 照单落实，未增删）**：
  - **（A）条件 18 半边无实跑证据**：`frontend`/`extension` 的计数与基线相等只有结构性论据，未观察。暂不修理由：两棵树零改动，跑两组对本 diff 无信息增益。**责任角色：coordinator。重评触发条件：下一个触及 `frontend/` 或 `extension/` 的任务（即 TASK-040）必须实跑两组并记录计数。** —— **已由 TASK-040 承接（其完成条件 14）。**
  ...
  - **（G）删除预览界面少报图片张数**：前端两份各自固定六键的解析器加一份标签表都忽略 `snapshot_asset_count`（不报错，只少报）。暂不修理由：当前不可达——还没有调用方能产生资产。**责任角色：TASK-040 的写入者。重评触发条件：硬性——扩展开始写资产之前必须改完四处（`frontend/src/api/client.ts` 的接口与键表、`frontend/src/features/resources/api.ts` 的键表、`ResourceDeletion.tsx` 的标签表），否则即成用户可见的删除漏报。** —— **已由 TASK-040 承接为硬性前置（其关键设计决定 ⑤、完成条件 11）。**
