...
- 实现 SHA/变更摘要（base ed36a22 起，均在本任务 allowed_paths 内，含用户批准增补）：
  - `408f80e` feat(backend)：models.py `LearningResource.title` 列可空（`Mapped[str | None]`，保留 `bounded_length("title",1,200)` CHECK——NULL 可通过、空串仍被 DB 拒绝，语义等价且更严）；contracts.py `CreateBase.title` 可选/显式 null（空白仍 422）、`ResourcePatch` 允许显式 null 清空标题；resource_store.py 按 title 排序时 null 固定排最后（决胜 id asc）；0003 迁移（含 FK-OFF 支撑：connection.py `migration_connection()`、migrations/env.py、database/__init__.py、tests/support.py `migrate()` 接线，运行时连接仍默认 FK ON）。
...
- 独立 Acceptance（第二个独立只读 reviewer 型实例，2026-09-05，报告原文）：
  > 结论 **ACCEPT** —— 完成条件 1-6 均有实现+测试绑定且自洽，跨模块/契约闭环，运行证据与候选指纹吻合，无 BLOCKED 级残留（无越界、无底线违背、无 NOT_RUN 冒充 PASS）。
  > 逐条：条件1 models.py:123 title `Mapped[str|None]` + 0003 batch nullable + FK-OFF 仅迁移连接（运行时 FK ON），test_migrations 绑 0001→head 保留附属笔记/空库 downgrade/compare_metadata==[]（484 pytest 含 6 迁移测试）；条件2 contracts CreateBase.title 可空/空白 422，file_upload.py:153 FILE 复用同一 FileCreate（title 缺省→null）；条件3 ResourcePatch 显式 null 清空 version+1、省略不改，source_url/pasted_content not_null 仍禁清空；条件4 resource_store.py:428-433 title 排序 null 固定最后 + id asc 决胜、q 对 null 走 `value or ""` 不命中；条件5 openapi 4 schema + 中文契约一致；条件6 前端占位「未命名资料」+ FILE 非空才 append + vitest 与新增两条 e2e 绑定。
  > 跨模块/运行证据：backend↔frontend↔docs 闭环；check029b.log `input=6b4f867…`、files=40、fingerprint=`1d68cdaa…` 与 EVIDENCE 一致，全 profiles exit=0、CHECKS PASS；日志中 `missing-tool`/`fake-test` NOT_RUN/FAIL 系 governance 自测演示失败检测（后接 23 tests OK），非冒充；e2e 4 条红为 main 基线复现一致、非本任务回归。
  > 可记录非阻断项：① 索引行 TASK-029 状态需同步（本次已随证据写回同步为 ACCEPTED）；② 任务单 prose 曾写 `bounded_length(…0,200)`，实际代码为 `1,200`（可空列下等价且更严，本次已修正）；③ backend FILE multipart 缺 title 直测缺失（后续补）；0003 downgrade 遇未命名行 IntegrityError 为已声明结构性限制。
<!-- EVIDENCE:END -->
