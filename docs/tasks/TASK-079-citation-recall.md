...
- 最终状态/风险/用户操作：**MERGED**——2026-09-21 用户已合并 PR #87，merge `9bd2b41`（状态登记并入 TASK-080 控制面提交）。合并后用户在 Edge 扩展页确认版本显示为 `0.2.0.632+b657284-dirty`，版本可判断这条已由用户实测通过。此前为 ACCEPTED：L3 执行链走完（Worker → 自动检查 → 独立只读 Reviewer **六轮** → 独立只读 Integration/Acceptance **四轮**；其中第五轮起是用户指令触发的范围修订重新走的链）。风险：改的是写进契约的识别门槛，放宽方向由四组假阳性反例守着；`version_name` 只是人看的标识，不扩大任何授权面。**需要用户操作**：
  1. **合并 PR**；
...
- **合并前的真实浏览器实测（2026-09-21，主 Agent 执行，用户要求「你自己去 edge 上测试」）**：用 **真实 Edge**（Playwright `channel: 'msedge'`，无头）打开**线上真实页面**，把**构建产物 `extension/dist/extract.js` 原样注入**（用 `chrome.runtime` 替身接住它发出的消息）后读取结果。这不是 jsdom、不是存档 HTML、不是源码——是装进浏览器的那一份 bundle 在真实页面上的行为。六个页面：

  | 页面 | 结果 | 判断 |
  | --- | --- | --- |
  | `arxiv.org/abs/1706.03762` | PREPRINT、8 位作者、2017、`10.48550/arXiv.1706.03762` | ✅ 正是用户报障的那一页 |
  | `arxiv.org/abs/2005.14165` | PREPRINT、2020、`10.48550/arXiv.2005.14165` | ✅ |
  | `arxiv.org/abs/1207.7214`（已发表于 Phys.Lett.B） | PREPRINT、DOI `10.1016/j.physletb.2012.08.020`、出处为空 | ⚠️ 见下方遗留 9 |
  | `nature.com/articles/s41586-021-03819-2` | JOURNAL_ARTICLE、Nature、2021、`10.1038/s41586-021-03819-2` | ✅ 真实期刊站的强信号路径 |
  | 维基百科 `Transformer (deep learning architecture)`（参考文献区大量 DOI 链接） | **不识别（null）** | ✅ **「不认领别人的 DOI」在真实页面上成立**——这正是第一轮 Review F1 的那条缺陷 |
  | `wordpress.org/news/2024/11/wordpress-6-7-rollins/`（真实 WordPress 站） | **不识别（null）** | ✅ 博客「往回拉」在真实页面上成立 |

  **这组实测消解了此前三条「只有推断、无真实页面证据」的剩余风险中的两条**（arXiv 真实页、带 Related DOI 的那一族）。仍未覆盖的是「用户在自己的 Edge 里点图标 → 确认页 → 保存」这条**完整交互链路**（popup 的 `activeTab` 注入无法用 Playwright 驱动），它仍需用户点一次。
  **版本这条也已由用户实测确认**：用户 2026-09-21 贴出 Edge 扩展页显示 `0.2.0.632+b657284-dirty`——**Edge 确实显示 `version_name`**。此前看到的光秃秃 `0.2.0` 是重载前的旧构建，不是 Edge 不显示。这条记在此处，免得把一个已被证伪的猜测留在记录里。
...
- 非阻断遗留项：
  ...
  8. **已发表的 arXiv 论文：类型仍判「预印本」、出处为空**（真实实测 `1207.7214` 发现）。arXiv 把 "Journal ref"（`Phys.Lett.B716:1-29,2012`）写在页面正文的表格里，**不发 `citation_journal_title`**，而本实现只读 meta 与 JSON-LD，所以读不到刊名；DOI 取到的是**已发表版的** `10.1016/…`（那确实是这份工作的 DOI，不是别人的，取值本身正确）。后果：一条已正式发表的文献在库里显示为「预印本 + 无出处」。要修就得读 arXiv 页面里那一行（属站点专用解析，本任务的非目标）。**重评触发**：用户明确希望已发表的 arXiv 论文显示为期刊论文时。
  9. **TASK-079 自己的 MERGED 登记**留给下一个任务（按用户 2026-09-20 的要求，登记类小活不单开 PR）。
