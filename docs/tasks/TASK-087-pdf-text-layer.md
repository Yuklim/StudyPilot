...
- `::selection` 的底色**留 65% 透明度**（`color-mix`，带 `rgba` 回退），作为 `mix-blend-mode`
  ...
  半透明这条（`color-mix(in srgb, AccentColor, transparent 75%)`）。顺带把 `br::selection`
  设成透明（官方同样）。**但 `mix-blend-mode: multiply` 是本项目自己的选择，不是上游做法**
  ——官方文字层没有这个属性（Review R4 提醒，免得后来人误以为是抄的）：官方用饱和的
  `AccentColor` 配 25% 透明，而本项目要保留淡黄 `--mark`，25% 压在白纸上等于看不见。
...
- **深色背景的页上选区几乎看不见**（第三轮 Review R1）：`multiply` 的固有代价——背景越暗，
  淡黄乘下去越接近原色。白纸上正常；深色版式的讲义或深色插图上选区会很淡。要两头都好看
  得按背景亮度切两套色，本任务不做。
...
- 候选 SHA：**最终候选 `f765b6b`**（base `198efc8`）。四次冻结：`ec3ec7e`（首次送审 → PASS +
  5 条非阻断）→ `d571a03`（处置四条 → PASS / No findings）→ `cc69d59`（用户实测「选区盖住字」
  的修复 → PASS + 4 条建议）→ `f765b6b`（采纳 R1–R4 → PASS / No findings）。
...
- Acceptance：N/A（L2）
- Review（第三轮，同一 Reviewer 增量复核 `d571a03..cc69d59`——用户实测缺陷的修复，**报告原文**）：

  > ## 结论：PASS（新候选 `cc69d59`，base 仍 `198efc8`）
  >
  > **只读证明**：仅 Read/Grep/Glob，无 Write/Edit、无 Bash；未改、未提交任何文件。
  >
  > **覆盖与继承**：只审 `d571a03..cc69d59`（3 文件，+125/−7）。注意这个区间实际含**两个**提交——`94e8da1`（写回两轮 Review 原文、候选 `d571a03`、status 置 ACCEPTED）与 `cc69d59`（本次修复）；diff 的净效果与「status/索引行回到 IN_REVIEW」自洽（两次改动相消，所以 TOML 与索引不出现在 diff 里），无隐藏改动。前两轮结论按 §6 继承。
  >
  > 1. **`mix-blend-mode` 无副作用，合成顺序不变**。它确实建立层叠上下文，但文字层本来就是 positioned（z-index auto），仍在 canvas 之后绘制。`overflow: clip` 与 `position:absolute; inset:0` 的定位完全不受影响。透明像素 `αs=0` 时结果恒等于背景，未选中区域一切照旧。
  > 2. **深色页确实会看不出选区**（你没测的那条，成立）。白纸下 multiply 与纯半透明同结果 ≈ `rgb(248,239,205)`；黑字下 `0.65·(0×244/255)+0.35·0 = 0`，字原样保留——这正是你要的。但背景本身是深色时（`Cb=30`）结果 ≈ 29，**选区几乎不可见**。这是 multiply 的固有代价，不是实现错误。建议登记为已知限制。
  > 3. **兜底兜得住，但偏弱，且基本是死代码**。不支持 multiply 时黑字上得到 `rgb(159,150,116)`，对比度约 **2.4:1**——字认得出（旧版是 0，整段抹掉）。Chrome/Edge/Safari/Firefox 自 2015 年起全支持 `mix-blend-mode`，维持现状合理。
  > 4. **像素守卫是真守卫**。你那次反证（637 → 0）是决定性的，它同时排除了另一件事：`.reader-quote` 是 `position: fixed; z-index: 7`，元素截图会连同浮层一起拍——若胶囊落进 page1 的框里，`after.dark` 会被抬高而掩盖回归；`dark=0` 证明那个区域里只有页面本身。阈值里**最弱的是 `after.marked > 200`**——绝对阈值，纸色若将来变成米黄会空转通过。
  >
  > **Findings**（无必须修复项）：R1 深色背景页选区不可见（建议登记）；R2 把黄色断言改成与 `before.marked` 比较；R3 `rgba(244,231,178,0.65)` 是 `--mark` 的手抄副本，改主题色会静默漂移；R4 记录里「官方 pdf.js 走的也是半透明这条」属实（`pdf_viewer.css:985-988`、`:994-996` 已逐字核对），但**官方并没有给文字层加 `mix-blend-mode`**，建议点明 multiply 是本项目的选择。
  >
  > **剩余风险**：深色页选区不可见；每个近视口页多一个混合合成层，长文档滚动重绘开销略增；`--total-scale-factor` 的对齐仍只有一条 0 度夹具的 e2e。

- Review（第四轮，同一 Reviewer 增量复核 `cc69d59..f765b6b`，**报告原文**）：

  > ## 结论：PASS（最终候选 `f765b6b`，base 仍 `198efc8`）
  >
  > **只读证明**：仅 Read/Grep/Glob，无 Write/Edit、无 Bash；未改、未提交任何文件。
  >
  > **覆盖与继承**：只审 `cc69d59..f765b6b`（3 文件，53 行）。经 `.git/logs/HEAD` 核实 `f765b6b` 是 `cc69d59` 的直接子提交，中间无隐藏提交。本轮改动为：一条 e2e 断言收紧、一段 CSS **纯注释**、记录三处措辞——不含任何运行时行为变更，故前三轮结论全部按 §6 继承，本结论覆盖最终候选 `f765b6b`。`files=11` 不变、指纹变化与「只改注释与断言」相符；styles.css 无声明变化，故前端 801 不需重跑亦成立。
  >
  > - **R2 已到位且严格更强**：`after.marked > before.marked + 200` 无论纸色如何变都不会空转，且 e2e 在本候选上实跑通过，反证了 `before.marked` 确实接近 0。
  > - **R3 保留回退是对的**，我不建议删。一处事实订正：没有那条 `rgba` 时，浏览器走的是**默认选区底色**（AccentColor 系的不透明蓝），不是「完全没有颜色」——在 multiply 下它其实也能保住黑字。保留与删除都安全，保留更可控。
  > - **R4 记录订正属实**；**R1 已入「已知限制」**，成因描述正确。
  >
  > **Findings：No findings**（无必须修复项，无新增非阻断项）。
  >
  > 收尾提醒（流程，非缺陷）：EVIDENCE 区把最终候选写实为 `f765b6b`、三轮 Review 原文照录、status 与 `docs/tasks/任务索引.md` 该行同步置 ACCEPTED；MERGED 只能在用户实际合并后登记。

- 主 Agent 对第三/四轮的处置：R1–R4 **四条全采纳**（R1 入已知限制、R2 断言改成与选中前比较、
  R3 回退色加注释说明它是 `--mark` 的手抄副本、R4 记录点明 multiply 是本项目的选择）。
  Reviewer 对 R3 的事实订正（没有 `rgba` 回退时走的是浏览器默认选区色，而非「完全没有颜色」）
  一并记在这里——我原先的表述不准确。

- 最终状态/风险/用户操作：**MERGED**——2026-09-22 用户合并 PR #95，merge 提交 `435f769`；
  合并前状态为 **ACCEPTED**（L2 执行链走完：实现 → 每轮机械检查 → 同一独立只读
  Reviewer **四轮**：首轮 PASS + 5 条非阻断（改掉四条）→ PASS → 用户实测缺陷修复后 PASS + 4 条
  建议（全采纳）→ PASS / No findings）。
  风险：只动前端渲染与交互，不碰后端/契约/数据；最坏情况是文字层没挂上或对不齐，canvas
  阅读不受影响（旋转页已主动降级为不挂）。
  **过程中被外部发现一次真缺陷**：四轮 Review 都没抓到「选区不透明底色盖住 canvas 上的字」，
  是**用户在自己库里的 arXiv 论文上实测发现的**。补的像素级 e2e 守卫已反证有效（637 → 0）。
  **已无待办**：用户实测通过文字层与「记下这段」后说「推pr吧」→ PR #95 → 用户合并。
  本条 MERGED 登记按 §5 并入 TASK-088 的控制面提交。
- 非阻断遗留项：
