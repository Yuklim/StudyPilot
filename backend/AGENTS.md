...
- TASK-003 的 `docs/contracts/API与数据契约基线.md` 与 `openapi-v1.json` 已由用户合并，业务数据含义以其为准；但具体运行行为仍须任务授权，不能据此直接开放所有接口。
- TASK-005 只建立数据库基础设施、初始模型与迁移；不改变 HTTP 行为，不在导入或应用启动时自动连接、建表或迁移。后续功能不能自行改表或绕开唯一迁移维护者。
