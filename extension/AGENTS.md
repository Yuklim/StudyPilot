...
- **当前已授予的 reach 就是全部，逐条有据**：`activeTab`（仅在用户点击图标后授予当前那一个标签页，用完即失效）、`scripting`（把提取脚本注入该标签页）、`storage`（popup 关闭后暂存一份待交付内容，交付即删）、`unlimitedStorage`（TASK-080；仅解除上一项的默认约 10 MB 配额，好暂存抓下来的 PDF，**不含任何站点访问权与网络可达范围**）、**一条只匹配 `http://127.0.0.1:5173/*` 的内容脚本**，以及 **`optional_host_permissions: ["<all_urls>"]`（TASK-040）**——后者**安装时不授予**，只在用户于 popup 点「连图片一并保存」时经 `chrome.permissions.request` 请求，可随时撤销，用于由 background service worker 取回正文图片的字节（`credentials: 'omit'`，不带任何站点凭证）。**不申请永久的 `host_permissions`**——那会把「用户主动指定的一页」换成「安装即持有、不再询问的长期访问权」；可选权限的覆盖面与它完全相同，差别只在授予时机与可撤销性。
- **权限最小化且需显式授权**：给扩展增加任何 reach —— `permissions`、`host_permissions`、`optional_permissions`、`content_scripts`、`externally_connectable`、`web_accessible_resources`、CSP 覆写等 —— 必须在任务记录中写明用途与替代方案，并经独立 Review。`src/manifest.test.ts` 断言 manifest 的顶层键恰好是既定的那一组（当前为 `manifest_version`/`name`/`version`/`version_name`/`description`/`permissions`/`optional_host_permissions`/`background`/`content_scripts`/`action` **十个**，TASK-040 新增 `optional_host_permissions` 与 `content_scripts`，TASK-079 新增 `version_name`），另有断言锁死 `permissions` 的确切四项（`activeTab`/`scripting`/`storage`/`unlimitedStorage`——**没有一项给出站点访问权**：manifest.test.ts 另有一条断言钉死 `permissions` 里不得出现 `://` 或 `<all_urls>`）、`optional_host_permissions` 恰好为 `["<all_urls>"]` 且不得出现 `host_permissions`、`background` 条目的确切键集、以及内容脚本的唯一匹配源，**任何**新增键都会让它失败；这是有意的白名单门闩，不得为通过测试而删除或放宽断言。用白名单而非逐个点名危险键，是因为点名必然漏掉下一个。
