...
StudyPilot 是一个面向个人学习者的资料与学习过程管理助手。目前可以在简约手帐风页面中保存网页链接和粘贴内容、搜索/筛选资料并阅读详情。学习进度操作、笔记、复习、统计、文件上传和 AI 尚未实现。
...
网页/粘贴资料页面已接入真实后端。没有示例资料冒充真实数据；未开放功能会明确提示，不放置无效操作按钮。

### 可以使用哪些页面（TASK-007 / TASK-010）

导航、返回和浏览器前后退可用；资料库、添加资料、资料详情已接入，其他页面仍为明确标记的功能预留。

| 页面 | 本地路径 |
| --- | --- |
| 学习概览 | `/` |
| 资料库 | `/resources` |
| 添加网页或粘贴资料 | `/resources/new` |
| 资料详情 | `/resources/:resourceId`（从资料卡片打开；不存在的编号显示受控提示） |
| 学习记录 | `/study-records` |
| 复习安排 | `/reviews` |
| 主题统计 | `/topics` |

直接访问或刷新这些地址仍能显示页面；其他地址会提示“没有找到这个页面”，并提供返回入口。统计用“— / 未接入”表示未连接数据，不是零记录。只有资料页面按需请求同源接口；不加载外部素材，不在浏览器持久化凭据、草稿或原文。键盘用户可按 Tab 找到“跳到主要内容”；切换页面后焦点会移到新标题。
...
草稿与筛选只在当前页面内存中；离开或刷新会清空。保存中不会重复提交；网络或响应错误可能无法确认是否已保存，请先查看资料库，避免重复添加。不会自动重试保存。文件上传、分类管理、修改/删除、学习操作尚未开放，初始学习状态为“未开始 · 0%”，不是虚构的学习记录。
...
用户已确认分阶段交付：当前可用范围见[契约第 1.3 节](docs/contracts/API与数据契约基线.md#13-task-009-阶段性交付与当前可用范围-用户确认架构)及 OpenAPI 的 `x-delivery-profile`。完整 OpenAPI 保留最终 MVP 目标，不表示所有分支现在都可用；通过安全检查的文件 multipart 暂返回 `415 CONTENT_TYPE_UNSUPPORTED`，不读取正文或保存文件。后续独立文件任务实现并验收后更新清单，最终文件要求不取消；不要直接据完整目标生成当前可用的上传入口。
...
当前后端仅支持引用已有主题/标签，分类管理接口以后实现；页面暂不提供分类选择器，省略主题/标签即可保存。文件 multipart、修改/删除、学习操作仍未开放。Unicode 搜索为保持跨数据库语义一致，在已筛选的元数据上执行内存匹配；不加载粘贴正文，适用于当前个人/低流量规模，资料规模显著增长后再按测量优化。每次显式业务请求使用独立数据库会话，结束关闭，不共享可变状态。
...
```bash
PYTHONDONTWRITEBYTECODE=1 backend/.venv/bin/python scripts/governance/check_task.py --task docs/tasks/TASK-010-resource-pages.md --worktree
```
...
当前后端提供 `/health`、受保护的 `/api/v1/local-session` 和三个资料接口，不提供根页面。404 的意思是“这个地址目前没有对应页面”，不是后端没有启动；界面在前端的 5173 端口。
...
全新数据库没有资料，可从“添加资料”保存第一份网页或粘贴内容。若显示连接/数据库错误，请按启动顺序检查后端和迁移；空库、搜索无结果与服务失败会分别提示。概览统计、学习、复习等尚未接入，不会用假数据填充。
...
TASK-003 已冻结契约；TASK-005～008 提供数据库、测试、手帐风页面和本机安全基础，TASK-009/010 实现网页/粘贴资料的后端与页面。后续继续分类管理、文件和学习功能；具体边界按下一任务登记。内容解析、模型 API Key、AI、RAG 和学习 Agent 仍属更晚阶段，不能因为目录已经存在就提前加入。
