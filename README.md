...
前端开发服务器提供同源 `/api` 代理。后续前端业务代码只能访问 `/api`，不能把后端端口写死在组件中；TASK-008 已实现既定安全协议与共享 JSON 客户端，但业务接口仍未实现。当前页面仍可单独启动前端浏览，无需运行后端或建库；它不会自动调用新客户端。
...
**安全连接不等于业务完成或公网认证。**目前唯一新增的成功 API 是 local-session，已授权请求到未实现业务路由仍为框架 404；没有读取或保存真实资料，也没有自动迁移数据库。禁止直接公网部署、开放多个账号或宣称业务闭环已通过。
...
当前验证：页面明确显示“业务功能尚未实现”、导航与前后退、资料详情直接访问/刷新、未知地址返回、键盘焦点，以及 390px/320px 窄屏无横向溢出；同时验证共享客户端真实 bootstrap、三种简单写请求拒绝、直接导航/跨站初始化拒绝、合法/非法预检经过后端校验，以及携带令牌后到达未实现路由的框架 404。原代理 403 断言保留并采用 TASK-003 已批准的错误结构。桌面与窄屏截图保存在 `frontend/test-results/`，供人工检查布局，不进 Git。安全套件关闭 trace，测试只输出布尔值/状态码，不保存真实临时令牌或请求头。**没有模拟业务成功，也不代表添加、学习、复习等主闭环已经通过。**只跑 Chromium、单 Worker、零自动重试；失败需要修正或说明原因，不能靠重试掩盖。
