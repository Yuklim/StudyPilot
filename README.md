...
StudyPilot 是一个面向个人学习者的资料与学习过程管理助手。目前已具备工程框架及首批资料后端接口：可以保存网页链接和粘贴内容，搜索/筛选资料、读取详情；前端仍是尚未接入业务数据的手帐风页面骨架。学习进度操作、笔记、复习、统计、文件上传和 AI 尚未实现。
...
后端已能保存/查询网页与粘贴资料，但**页面尚不能操作资料库或添加资料**。页面不会展示假的业务数据或无效的业务按钮；真实表单和数据接入由后续任务完成。
...
前端开发服务器提供同源 `/api` 代理。前端业务代码只能访问 `/api`，不能把后端端口写死在组件中；TASK-008 提供安全协议与共享 JSON 客户端，TASK-009 提供首批资料接口。当前页面仍可单独启动前端浏览，无需运行后端或建库；它不会自动调用客户端，上面的页面提示指界面仍未接入。
...
本次完成三个接口：`POST /api/v1/resources` 保存 JSON 网页/粘贴资料；`GET /api/v1/resources` 分页搜索/筛选/排序；`GET /api/v1/resources/{id}` 读取详情。仍需上述本机访问保护。页面暂未接入，无需你手动复制临时令牌尝试。
...
当前仅支持引用已有主题/标签，分类管理接口以后实现；省略主题/标签即可保存。文件 multipart、修改/删除、学习操作和界面表单不在本次范围。Unicode 搜索为保持跨数据库语义一致，在已筛选的元数据上执行内存匹配；不加载粘贴正文，适用于当前个人/低流量规模，资料规模显著增长后再按测量优化。每次显式业务请求使用独立数据库会话，结束关闭，不共享可变状态。
...
```bash
PYTHONDONTWRITEBYTECODE=1 backend/.venv/bin/python scripts/governance/check_task.py --task docs/tasks/TASK-008-local-access-foundation.md --worktree
```
...
当前验证：页面明确提示尚未接入、导航与前后退、资料详情直接访问/刷新、未知地址返回、键盘焦点，以及 390px/320px 窄屏无横向溢出；同时验证真实 bootstrap、非法写入/导航/跨站初始化拒绝、后端预检，以及未知路径 404。TASK-009 增加通过真实客户端保存 WEB/PASTE、刷新页面后搜索列表和读取详情，不模拟业务成功。原安全断言保留。截图保存在 `frontend/test-results/`，不进 Git。安全和资料套件关闭 trace，不保存真实临时令牌或请求头，资料测试只用合成内容。**已验证接口链路，不代表页面表单、学习或复习闭环完成。**只跑 Chromium、单 Worker、零自动重试；失败需要修正或说明原因，不能靠重试掩盖。
...
当前后端提供 `/health` 与受保护的 `/api/v1/local-session`，根地址和业务 API 尚未实现。404 的意思是“这个地址目前没有对应页面”，不是后端没有启动。
...
TASK-003 已冻结契约；TASK-005～008 提供数据库、测试、手帐风页面和本机安全基础，TASK-009 实现首批资料后端。下一步把网页/粘贴资料的表单、列表和详情接入现有页面；分类管理、文件和学习功能随后按任务推进。内容解析、模型 API Key、AI、RAG 和学习 Agent 仍属更晚阶段，不能因为目录已经存在就提前加入。
