...
| 声明 | 何时生效 | 用途 |
| --- | --- | --- |
| `activeTab` | **只在你点击扩展图标之后**，授予当前那一个标签页，用完即失效 | 读取你眼前这一页**已经显示出来**的内容 |
| `scripting` | 同上 | 把提取脚本注入上面那一个标签页 |
| `storage` | 始终 | popup 打开新标签页后就被关闭，采集结果需先落到扩展存储里再转交 |
| `http://127.0.0.1:5173/*`（内容脚本） | 始终 | **只匹配本机 UI 这一个地址**，用于把正文交给 `/capture` 确认页 |
...
扩展不读 cookie、不接触任何网站的登录态；取图时也不带站点凭证（`credentials: 'omit'`），所以**需要登录才可见的图片取不到**——这是设计而非缺陷。`extension/src/manifest.test.ts` 用白名单锁住 manifest 的顶层键与权限清单，任何新增权限都会让测试失败，必须经过任务与独立审查。
